...
describe('fetchAvailableModels', () => {
    ...
    it('returns empty array if Gemini provider lacks API key', async () => {
        const result = await fetchAvailableModels({ baseURL: 'https://generativelanguage.googleapis.com', apiKey: '' })
        expect(result).toEqual([])
        expect(global.fetch).not.toHaveBeenCalled()
    })
    ...
    it('returns empty array if response is not ok', async () => {
        ...
        const result = await fetchAvailableModels({ baseURL: 'https://api.openai.com/v1', apiKey: 'bad' })
        expect(result).toEqual([])
    })
    ...
    it('returns empty array for unexpected data structure', async () => {
        ...
        const result = await fetchAvailableModels({ baseURL: 'https://api.openai.com/v1', apiKey: 'test' })
        expect(result).toEqual([])
    })
