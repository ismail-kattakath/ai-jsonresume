...
describe("Language Component", () => {
  ...
  describe("Display Section Toggle", () => {
    it("should reflect showLanguages state in checkbox", () => {
      ...
      const checkbox = screen.getByLabelText("Display Section") as HTMLInputElement;
  ...
  describe("Layout and Styling", () => {
    ...
    it("should have hover states on language containers", () => {
      ...
      const languageContainer = container.querySelector(".hover\\:border-white\\/20");
      expect(languageContainer).toBeInTheDocument();
    });

    it("should have gradient accent on section heading", () => {
      const { container } = renderWithContext(<Language />);
      const gradient = container.querySelector(".bg-gradient-to-b.from-emerald-500.to-teal-500");
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle long language names", () => {
      const longName = "Language with a very long name that exceeds normal length";
      const mockData = createMockResumeData({
        languages: [longName],
      });
      renderWithContext(<Language />, {
