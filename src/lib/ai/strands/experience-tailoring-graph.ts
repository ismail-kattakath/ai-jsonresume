...
export async function tailorExperienceToJDGraph(
    ...
    onProgress?.({ content: 'Analyzing job requirements and experience fit...', done: false })
    ...
    onProgress?.({ content: 'Tailoring description to job requirements...', done: false })
    ...
    onProgress?.({ content: 'Optimizing achievements for relevance...', done: false })
    ...
    let rewrittenAchievements = achievementsResult.toString().trim().split('\n').filter(a => a.trim())
