...
jest.mock('framer-motion', () => ({
    motion: {
        div: ({ children, initial, animate, transition, viewport, ...props }: any) => (
            <div {...props}>{children}</div>
        ),
        h1: ({ children, initial, animate, transition, ...props }: any) => (
            <h1 {...props}>{children}</h1>
        ),
        h2: ({ children, initial, animate, transition, ...props }: any) => (
...
jest.mock('@/components/layout/MainLayout', () => {
    return function MockMainLayout({ children, ...props }: any) {
        return <div data-testid="mock-main-layout" {...props}>{children}</div>
    }
})
