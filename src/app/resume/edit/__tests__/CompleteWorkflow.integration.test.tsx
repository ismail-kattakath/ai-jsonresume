...
describe('Integration: Complete Edit → Preview → Print Workflow', () => {
  ...
  describe('Basic Edit → Preview Workflow', () => {
    it('should allow user to create a resume from scratch and see live preview', async () => {
      ...
      fireEvent.change(nameInput, { target: { name: 'name', value: 'Alice Johnson' } });
      ...
      const addWorkButton = screen.getByText(/Add Work Experience/i).closest('button');
      if (addWorkButton) {
    ...
    it('should maintain data consistency throughout the edit workflow', async () => {
      ...
      const nameInput = container.querySelector('input[name="name"]') as HTMLInputElement;
      fireEvent.change(nameInput, { target: { name: 'name', value: 'Bob Smith' } });
      ...
      await waitFor(() => {
        ...
        expect(preview).toHaveTextContent('Experienced professional with a passion for innovation');
      });
  ...
  describe('Import → Edit → Preview Workflow', () => {
    ...
    it('should allow adding new sections to imported resume', async () => {
      ...
      const addEducationButton = screen.getByText(/Add Education/i).closest('button');
  ...
  describe('Export Workflow', () => {
    it('should allow exporting edited resume as JSON', async () => {
      ...
      // Edit some data
      const nameInput = container.querySelector('input[name="name"]') as HTMLInputElement;
      fireEvent.change(nameInput, { target: { name: 'name', value: 'Export Test User' } });
  ...
  describe('Print Workflow', () => {
    it('should display print button that is accessible throughout editing', async () => {
      ...
      // Make some edits
      const nameInput = container.querySelector('input[name="name"]') as HTMLInputElement;
      fireEvent.change(nameInput, { target: { name: 'name', value: 'Print Test User' } });
    ...
    it('should show formatted preview ready for printing', async () => {
      ...
      const nameInput = container.querySelector('input[name="name"]') as HTMLInputElement;
      fireEvent.change(nameInput, { target: { name: 'name', value: 'Print Ready User' } });
  ...
  describe('Complex Multi-Step Workflow', () => {
    it('should handle complete resume creation from blank to print-ready', async () => {
      ...
      const nameInput = container.querySelector('input[name="name"]') as HTMLInputElement;
      fireEvent.change(nameInput, { target: { name: 'name', value: 'David Martinez' } });
      ...
      await waitFor(() => {
        ...
        // Summary
        expect(preview).toHaveTextContent('Passionate developer with 5 years of experience');

        // Work Experience
        expect(preview).toHaveTextContent('Tech Solutions Inc');
        expect(preview).toHaveTextContent('Senior Developer');
        expect(preview).toHaveTextContent('Led development of microservices architecture');
    ...
    it('should allow editing, reviewing in preview, making corrections, and printing', async () => {
      ...
      const nameInput = container.querySelector('input[name="name"]') as HTMLInputElement;
      fireEvent.change(nameInput, { target: { name: 'name', value: 'Emma Wilson' } });
      ...
      fireEvent.change(nameInput, { target: { name: 'name', value: 'Emma Wilson PhD' } });
  ...
  describe('Responsive Preview Behavior', () => {
    it('should maintain preview visibility during editing on desktop layout', async () => {
      ...
      // Make an edit
      const nameInput = container.querySelector('input[name="name"]') as HTMLInputElement;
      fireEvent.change(nameInput, { target: { name: 'name', value: 'Responsive Test' } });
  ...
  describe('Data Persistence During Workflow', () => {
    it('should maintain all data when user adds multiple sections sequentially', async () => {
      ...
      // Add personal info
      const nameInput = container.querySelector('input[name="name"]') as HTMLInputElement;
      fireEvent.change(nameInput, { target: { name: 'name', value: 'Frank Chen' } });
