...
export async function generateSummaryGraph(
    ...
    if (onProgress) onProgress({ content: 'Identifying job-relevant metrics and skills...\n', done: false })
    ...
    if (onProgress) onProgress({ content: 'Drafting your professional summary...\n', done: false })
    ...
    while (iteration <= maxIterations) {
        ...
        const writerPrompt = iteration === 1
...
/**
 * Internal skill validation helper
 */
function validateSkillsInSummary(summary: string, allowedSkills: string[]) {
    ...
    const ignoreList = new Set(['the', 'and', 'for', 'with', 'years', 'experience', 'senior', 'lead', 'engineer', 'developer', 'systems', 'solutions', 'scalable', 'building', 'expert', 'specializing', 'architecting', 'architected', 'focusing', 'align', 'innovation', 'impact', 'production'])
    const violations: string[] = []

    mentions.forEach(mention => {
        if (mention.length < 3 || ignoreList.has(mention)) return

        const isAllowed = Array.from(normalizedAllowed).some(allowed =>
            allowed === mention || allowed.includes(mention) || mention.includes(allowed)
        )
