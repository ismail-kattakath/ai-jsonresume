import { sortAchievementsGraph } from '../achievements-sorting-graph'
        ...
        Agent: jest.fn().mockImplementation(({ systemPrompt }: any) => ({
            invoke: jest.fn().mockImplementation((prompt: string) => {
                if (systemPrompt.includes('identifying what makes achievements relevant')) {
                    ...
                    return Promise.resolve({ toString: () => '{"rankedIndices": [1, 0]}' })
...
describe('achievements-sorting-graph', () => {
    ...
    it('should sort achievements successfully on first try', async () => {
        const result = await sortAchievementsGraph(
            achievements,
            'Dev',
            'Company',
            'JD',
