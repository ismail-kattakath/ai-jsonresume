 ...
 */
export const DnDContext: React.FC<DragDropContextProps> = ({
  children,
  ...props
}) => {
 ...
 */
export const DnDDroppable: React.FC<DroppableProps> = ({
  children,
  ...props
}) => {
 ...
 */
export const DnDDraggable: React.FC<DraggableProps> = ({
  children,
  ...props
}) => {
      ...
      {(provided, snapshot) => (
        <div
          ...
          className={`group flex cursor-grab flex-col gap-3 rounded-lg border border-white/10 bg-white/5 p-4 hover:border-white/20 hover:bg-white/10 active:cursor-grabbing ${
            snapshot.isDragging
              ? `bg-white/20 outline-2 ${outlineColorClass} outline-dashed`
              : ''
          } ${className}`}
        >
          {children}
